...
static TokenType check_keyword(const Lexer *lex, short start, short len, const char *rest,
                               TokenType type) {
...
void lex_consume(Lexer *lex, Token *t) {
    ...
    case '}':
        ...
        return;
    case '[':
        token(lex, TOKEN_LEFT_BRACKET, t);
        return;
    case ']':
        token(lex, TOKEN_RIGHT_BRACKET, t);
        return;
    case ':':
...
void lex_print(const Lexer *lex) {
    ...
    for (;;) {
        ...
        if ((token.end - token.start) < INT_MAX) {
            ...
            printf("%17s %2d %.*s...\n", TOKEN_TO_STRING[token.type], token.type, INT_MAX,
                   token.start);
        }
